...
//mytest code
  function createRankList($user_list){
    ...
    $temp_uid = array();
    for($i=0;$i<count($user_list);$i++){
      $qry="SELECT count(*) FROM `daily_update` WHERE `uid`='". $user_list[$i]->id ."' ;";
      $flag=mysqli_query($link,$qry) or die (mysqli_error($link));
      while($row = mysqli_fetch_array($flag, MYSQLI_ASSOC)) {
          $count=$row["count(*)"];
      }
      if($count > 0){
          $count = $count - 1;
          $qry2="SELECT uid,points FROM `daily_update` WHERE `uid` = '".$user_list[$i]->id."' LIMIT ".$count.", 1;";
          $flag2=mysqli_query($link,$qry2) or die (mysqli_error($link));
          while($row2 = mysqli_fetch_array($flag2, MYSQLI_ASSOC)) {
            $temp_points[$i] = $row2["points"];
            $temp_uid[$i] = $row2["uid"];
          }
      }
    }

    $qry3="SELECT count(*) FROM `user_rank_list`;";
    $flag3=mysqli_query($link,$qry3) or die (mysqli_error($link));
    while($row = mysqli_fetch_array($flag3, MYSQLI_ASSOC)) {
        $count=$row["count(*)"];
    }
    if($count > 0){
      $qry4="TRUNCATE `user_rank_list`;";
      mysqli_query($link,$qry4) or die (mysqli_error($link));

      for ($i=0; $i < count($temp_uid); $i++) {
        $qry5="INSERT INTO `user_rank_list` VALUES ('".$temp_uid[$i]."', $temp_points[$i]);";
        $flag5=mysqli_query($link,$qry5) or die (mysqli_error($link));
      }
    }
    else{
      for ($i=0; $i < count($temp_uid); $i++) {
        $qry6="INSERT INTO `user_rank_list` VALUES ('".$temp_uid[$i]."', $temp_points[$i]);";
        $flag6=mysqli_query($link,$qry6) or die (mysqli_error($link));
      }
    }
  }
  //mytest code
